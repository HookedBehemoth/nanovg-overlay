...
BUILD		:=	build
SOURCES		:=	source nanovg-deko3d/source nanovg-deko3d/source/framework
DATA		:=	data
INCLUDES	:=	include nanovg-deko3d/include/ nanovg-deko3d/include/nanovg nanovg-deko3d/include/nanovg/framework
#ROMFS		:=	romfs

# Output folders for autogenerated files in romfs
SHADERS	:=	nanovg-deko3d/shaders
...
clean:
	@echo clean ...
ifeq ($(strip $(APP_JSON)),)
	@rm -fr $(BUILD) $(DATA_FOLDERS) $(TARGET).ovl $(TARGET).nacp $(TARGET).elf
else
	@rm -fr $(BUILD) $(DATA_FOLDERS) $(TARGET).nsp $(TARGET).nso $(TARGET).npdm $(TARGET).elf
endif
...
ifeq ($(strip $(NO_NACP)),)
$(OUTPUT).ovl		:	$(OUTPUT).elf $(OUTPUT).nacp $(DATA_DEPS)
	@elf2nro $< $@ $(NROFLAGS)
	@echo "built ... $(notdir $(OUTPUT).ovl)"
else
$(OUTPUT).ovl	:	$(OUTPUT).elf $(DATA_DEPS)
endif
